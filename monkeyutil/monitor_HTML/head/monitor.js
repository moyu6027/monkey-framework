	...
	switch (a){
		case 'free':
			...
			for (var i=0; i < data[0].length; i++){
				free.push([data[1][i]]);
				memfree.push([data[2][0][i]]);
				buffers.push([data[2][1][i]]);
				cached.push([data[2][2][i]]);
				if(data[1].length == 4){CMA.push([data[2][3][i]])}
			}
		...
		case 'all':
			...
			for (var i=0; i < data[0].length; i++){
				free.push([data[1][i]]);
				active.push([data[2][i]]);
				inactive.push([data[3][i]]);
				io.push([data[4][i]]);
				mapped.push([data[5][i]]);
				slab.push([data[6][i]])
			}
			series.push({name:'Available_Memory',data:free});
		...
		var active=[],inactive=[],active_a=[],inactive_a=[],active_f=[],inactive_f=[];
			for (var i=0; i < data[0].length; i++){
				active.push([data[1][i]]);
				inactive.push([data[2][i]]);
				active_a.push([data[3][i]]);
				inactive_a.push([data[4][i]]);
				active_f.push([data[5][i]]);
				inactive_f.push([data[6][i]]);
			}
			series.push({name:'Active',data:active});
			series.push({name:'Inactive',data:inactive});
...
function getcurfreq(){
	...
	for (var j=0; j < curfreqdata[1].length; j++){
		...
		for (var i=0; i < curfreqdata[0].length; i++){
			...
			tmp.push([curfreqdata[1][j][i]]);
		}
		series.push({name:'cpu' + j,data:tmp});
	}
	return [curfreqdata[0], series]
}

function getthermal(a){
	var tmp=[];
	for (var i=0; i < thermaldata[0].length; i++){
		tmp.push([thermaldata[2][a][i]]);
	}
	return [thermaldata[0], [{name:thermaldata[1][a],data:tmp}]]
}
...
function getFPS2(){
	var HandleInputStart=[],AnimationStart=[],PerformTraversalsStart=[],DrawStart=[],SyncQueued=[],SyncStart=[],IssueDrawCommandsStart=[],
	FrameCompleted=[];
	for (var i=0; i < fps_gfxinfo_data[0].length; i++){
		HandleInputStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[1][i]]);
		AnimationStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[2][i]]);
		PerformTraversalsStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[3][i]]);
		DrawStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[4][i]]);
		SyncQueued.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[5][i]]);
		SyncStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[6][i]]);
		IssueDrawCommandsStart.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[7][i]]);
		FrameCompleted.push([fps_gfxinfo_data[0][i],fps_gfxinfo_data[8][i]]);
	}
	var series=[];
	series.push({name:'HandleInputStart',data:HandleInputStart});
	series.push({name:'AnimationStart',data:AnimationStart});
	series.push({name:'PerformTraversalsStart',data:PerformTraversalsStart});
	series.push({name:'DrawStart',data:DrawStart});
	series.push({name:'SyncQueued',data:SyncQueued});
	series.push({name:'SyncStart',data:SyncStart});
	series.push({name:'IssueDrawCommandsStart',data:IssueDrawCommandsStart});
	series.push({name:'FrameCompleted',data:FrameCompleted});
	return [fps_gfxinfo_data[0][0],series]
}
function getGFX(a){
	var fps=[], Score=[], OKTF=[];
	var Surface = fps_systrace_data[1][16];
	var A = fps_systrace_data[1][11];
	var frames = fps_systrace_data[1][6];
	var TX = fps_systrace_data[1][15];
	var start_time = fps_systrace_data[1][2];
	var FPS = fps_systrace_data[1][4];
	var end_time = fps_systrace_data[1][3];
	var score = fps_systrace_data[1][5];
